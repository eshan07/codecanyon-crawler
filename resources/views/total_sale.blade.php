        ...
        <table class="w-full bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden text-center">
            ...
            @foreach ($salesByDatePaginated as $sale)
                <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
                    <td class="px-4 py-2 text-blue-600 hover:underline cursor-pointer" onclick="navigateTo('{{ route('project-wise-data', ['date' => $sale['date']]) }}')">{{ \Carbon\Carbon::parse($sale['date'])->format('d-m-Y') }}</td>
